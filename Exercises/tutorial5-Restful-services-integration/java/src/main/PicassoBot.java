...
public class PicassoBot {
  ...
  static String serverAddress = "http://ucx.dei.uc.pt";
      ...
      while ((cmd = in.readLine()) != null) {
        if (cmd.equals("tiles")) {
          ...
          if (cmdArgs.length == 3) {
            ...
            boolean res = picasso.deleteTile(x, y);
            if (res == true) {
              System.out.println("Deleted the tile!");
            }
          }
          ...
          if (cmdArgs.length == 6) {
            ...
            boolean res = picasso.editTile(x, y, r, g, b);
            if (res == true) {
              System.out.println("Edited the tile!");
            }
          }
  ...
  private boolean deleteTile(int x, int y) {
    try {
        URL url = URI.create(serverAddress + "/game/board/" + x + "/" + y).toURL();
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();

        // Use DELETE method to remove the tile
  ...
  private boolean editTile(int x, int y, int r, int g, int b) {
    try {
        URL url = URI.create(serverAddress + "/game/board/" + x + "/" + y).toURL();
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();

        // Use GET to check if the tile exists
        connection.setRequestMethod("GET");
        connection.setInstanceFollowRedirects(false);
        connection.setRequestProperty("Accept", "application/xml");
        connection.setRequestProperty("User-agent", "Pablo v1");
        ...
        if (responseCode == 404) {
            // Tile does not exist, call paintDot (POST)
            System.out.println("Tile does not exist. Creating a new tile with POST.");
            return paintDot(x, y, r, g, b);
        }

        // Parse the response to check the tile's state
        InputSource inputSource = new InputSource(connection.getInputStream());
        XPathFactory factory = XPathFactory.newInstance();
        XPath xPath = factory.newXPath();

        // Check if the tile is marked as "created"
        String created = xPath.evaluate("/tile/@created", inputSource);
        connection.disconnect();

        if (!"true".equals(created)) {
            // Tile exists but is not fully initialized, call paintDot (POST)
            System.out.println("Tile exists but is not initialized. Creating a new tile with POST.");
            return paintDot(x, y, r, g, b);
        }

        // Tile exists and is initialized, proceed with PUT to edit it
        ...
        // Pass the new values for r, g, and b in the request body
        OutputStream os = connection.getOutputStream();
        os.write(("r=" + r + "&g=" + g + "&b=" + b).getBytes());
        os.flush();
